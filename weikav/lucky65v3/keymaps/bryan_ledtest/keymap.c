#include QMK_KEYBOARD_H

#ifdef RGB_MATRIX_BLINK_ENABLE
#    include "rgb_matrix_blink.h"

// Required blink_rgbs array (unused in test but needed for linking)
blink_rgb_t blink_rgbs[RGB_MATRIX_BLINK_COUNT] = {
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
    {.index = 0xFF, .interval = 0, .times = 0, .color = {.r = 0x00, .g = 0x00, .b = 0x00}, .blink_cb = NULL},
};
#endif

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
        KC_ESC,  KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  KC_BSPC, KC_NO,   KC_DEL,
        KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC, KC_BSLS, KC_MUTE,
        KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT, KC_ENT,           KC_PGUP,
        KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,          KC_UP,   KC_PGDN,
        KC_LCTL, KC_LGUI, KC_LALT,                            KC_SPC,  KC_SPC,  KC_SPC,           KC_RALT, KC_RCTL, KC_LEFT, KC_DOWN, KC_RGHT
    ),
};

// LED test state
static uint8_t current_led = 0;
static uint16_t led_timer = 0;

void matrix_init_user(void) {
    // Turn off all LEDs initially
    rgb_matrix_mode(RGB_MATRIX_SOLID_COLOR);
    rgb_matrix_sethsv(0, 0, 0);
}

void matrix_scan_user(void) {
    // Cycle through LEDs every 1000ms (1 second each)
    if (timer_elapsed(led_timer) > 1000) {
        led_timer = timer_read();
        
        // Turn off all LEDs
        for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
            rgb_matrix_set_color(i, 0, 0, 0);
        }
        
        // Light up current LED in bright white
        rgb_matrix_set_color(current_led, 255, 255, 255);
        
        // Move to next LED
        current_led++;
        if (current_led >= RGB_MATRIX_LED_COUNT) {
            current_led = 0;
            // Brief pause between cycles
            wait_ms(2000);
        }
    }
}

const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    [0] = { ENCODER_CCW_CW(KC_VOLD, KC_VOLU) },
};
